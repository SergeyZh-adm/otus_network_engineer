...
a.	Выполним команду **no spanning-tree vlan 1 cost 18** режима конфигурации интерфейса, чтобы удалить запись стоимости, созданную ранее и посмотрим результат выполнения команды.
```
SW3(config)#
SW3(config)#int fa0/4
SW3(config-if)#
SW3(config-if)#no spanning-tree vlan 1 cost 18
SW3(config-if)#
SW3(config-if)#
SW3(config-if)#do sh spanning-tree
  Spanning tree enabled protocol ieee
...
Протокол STP сбросил порт fa0/2 на коммутаторе SW3 некорневого моста, вернув исходные настройки порта. Протоколу STP требуется примерно 30 секунд, чтобы завершить процесс перевода порта.
...
Активируем  избыточные пути до каждого из коммутаторов, чтобы просмотреть, каким образом протокол STP выбирает порт с учетом приоритета портов.

* Включим порты F0/1 и F0/3 на всех коммутаторах командой **no shutdown**  режима конфигурации интерфейса.

* Посмотрим как изменились роли и состояние портов на всех коммутаторах.
...
Не всегда протокол STP оптимально выбирает корневой мост в конкретной топологии. В нашем случае корневым мостом выбран коммутатор SW1. При этом пакеты от PC0 до маршрутизатора R1 будут проходить по пути PC0-SW2-SW1-SW3-R1 и обратно по этому пути.  
Более короткий путь прохождения пакетов - PC0-SW2-SW3-R1. Для этого нужно назначить корневым мостом коммутатор SW3, а коммутатор SW1 назначить резервным корневым мостом на случай аварии основного.  
...
Путь прохождения пакетов от PC0 до R1 стал оптимальным.
