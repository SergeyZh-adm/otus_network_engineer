...
|  R1        | G0/0/1.30    | 192.168.30.1 |255.255.255. |  -----           |
|  R1        | G0/0/1.1000  | -----        |  -----      |  -----           |
|  SW1       | VLAN 10      | 192.168.10.11|255.255.255.0|192.168.10.1      |
|  SW2       | VLAN 10      | 192.168.10.12|255.255.255.0|192.168.10.1      |
...
## Задачи
[1. Создание сети и настройка основных параметров устройства.](README.md#1-создание-сети-и-настройка-основных-параметров-устройства1)

[2. Создание сетей VLAN и назначение портов коммутатора.](README.md#2-создание-сетей-vlan-и-назначение-портов-коммутатора)

[3. Конфигурация магистрального канала стандарта 802.1Q между коммутаторами.](README.md#3-конфигурация-магистрального-канала-стандарта-8021q-между-коммутаторами)

[4. Настройка маршрутизации между сетями VLAN.](README.md#4-настройка-маршрутизации-между-сетями-vlan)

[5. Проверка работы маршрутизации между VLAN.](README.md#5-проверьте-работает-ли-маршрутизация-между-vlan)
...
В результатах вывода команды **tracert** видим, что отображается промежуточный IP адрес субинтерфейса gig0/0/1.30 роутера. Через него идет межсетевое взаимодействие VLAN 30, куда входит PC2 (шлюз по умолчанию для PC2).


>Если произодет авария с магистральной линией маршрутизатора или обесточивание маршрутизатора, то пропадет связанность хостов в различных VLAN. Аварийный  доступ к коммутаторам можно получить с ПК PC5, который входит в VLAN 10 Управления и способен связаться с коммутаторами даже в отсутствии межсетевой маршрутизации.



Пример рабочей конфигурации коммутатора SW1 приведен [здесь](/labs/lab06/configs/config%20SW1_06.txt)

Пример рабочей конфигурации коммутатора SW2 приведен [здесь](/labs/lab06/configs/config%20SW2_06.txt)

 Пример рабочей конфигурации маршрутизатора R1 приведен [здесь](/labs/lab06/configs/config%20R1_06.txt)