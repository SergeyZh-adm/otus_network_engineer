...
!
!Запрещаем доступ к интерфейсу gig0/0/1.10 (IP- 10.10.0.1/24) маршрутизатора R1 по протоколу HTTP и HTTPS
R1(config-ext-nacl)# deny tcp 10.40.0.0 0.0.0.255 host 10.10.0.1 range www 443
!
!Запрещаем доступ к интерфейсу gig0/0/1.30 (IP- 10.30.0.1/24) маршрутизатора R1 по протоколу HTTP и HTTPS
R1(config-ext-nacl)# deny tcp 10.40.0.0 0.0.0.255 host 10.30.0.1 range www 443
!
!Запрещаем доступ к интерфейсу gig0/0/1.40 (IP- 10.40.0.1/24) маршрутизатора R1 по протоколу HTTP и HTTPS
...
R1#
```
В ACL SALES  нет запрещающего правила для WEB трафика на интерфейсе gig0/0/1.20, по скольку ранее прописано правило запрещающее подключение ко всей сети 10.20.0.0/24 по протоколу HTTP и HTTPS.
